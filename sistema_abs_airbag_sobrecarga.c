...
void *tempo_airbag(void *arg) {
    ...
    while (1) {
        ...
        if (airbag_ativo) {
            ...
            // Simulação de sobrecarga: realiza cálculos desnecessários
            for (volatile int i = 0; i < 1000000; i++) {
                int x = i * i; // Cálculo sem efeito
            }

            // Simulação de sobrecarga: espera adicional
            usleep(100000); // Dorme por 100ms, forçando um atraso
